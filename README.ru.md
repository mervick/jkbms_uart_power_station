...
- **Умное отключение**: Отслеживает ток и SOC в реальном времени, активируя реле для полного отключения зарядки.
...
> [!CAUTION]  
> ⚠️ **ВЫСОКОЕ НАПРЯЖЕНИЕ на разъеме BMS UART!**  
> 
> Порт связи BMS (где находятся контакты UART) включает в себя пин, подключенный > напрямую к плюсу батареи (B+).  
> Этот пин несет **ПОЛНОЕ напряжение аккумулятора** (например, 12V, 24V, 48V+ и выше).
> 
> **НИКОГДА не подключайте его к Arduino!** Оставьте его неподключенным или полностью **обрежьте этот провод**.  
